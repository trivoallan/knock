...
- **`ports/`** — `typing.Protocol` interfaces only. **Must never import from `houba.adapters.*`.** Each port has a frozen-dataclass data model alongside it. The ports are `RegistryPort` (with `ImageInfo`), `ImageBuilderPort` (with `BuildRequest`), `Reporter` (with `Counts` / `ErrorInfo` / `OperationEvent`), `ClockPort`, `AttestorPort` (with `AttestationRef`), `SbomGeneratorPort` (with `SbomDocument`), and `UsageOraclePort` (with `UsageQuery` / `UsageObservation`).
