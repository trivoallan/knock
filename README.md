...
> signed SLSA / in-toto attestations** (rebuild *and* ingested scan results); a **package-level SPDX
> SBOM on every rebuilt image**; the
...
attestations). Every image houba rebuilds also carries a **package-level SPDX SBOM**, generated by
buildkit during the build houba controls.
...
names the owner — not a frantic spreadsheet. houba produces the provenance; your tools (Datadog, Wiz,
