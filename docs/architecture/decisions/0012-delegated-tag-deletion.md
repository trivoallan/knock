...
When a tag falls out of a policy, knock hard-deleted it immediately. But knock has no
...
external system, not knock.

## Decision

Add a deletionMode resolved through a most-specific-wins cascade (policy → destination →
global, default purge). In `mark` mode, knock attaches a `pending-deletion` OCI referrer
(`application/vnd.knock.lifecycle.pending+json`) instead of deleting: the image digest stays
...
knock stays a signal emitter for delegated deletion; `purge` mode is unchanged and remains
