...
published. The README Install section already promises `docker pull ghcr.io/.../houba`, an
unfulfilled doc/reality gap. houba's thesis is "the label is the product", and it ships signed
...
Publish on every release, as a multi-arch (`amd64` + `arm64`) image to `ghcr.io/<owner>/houba`, via a
...
- Approaches not taken: GitHub `attest-*` Sigstore-signed attestations (heavier, two ecosystems) and
  houba dogfooding its own image (circular). `edge` / dev images are out of scope.
