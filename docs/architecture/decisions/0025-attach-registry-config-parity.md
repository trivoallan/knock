...
`houba attach` was the one command in the fleet whose registry access was not governed by the
declared `HOUBA_REGISTRIES` roster: it called `registry.inspect` / `put_referrer` straight on
...
`attach` now authenticates via the `HOUBA_REGISTRIES` roster: by default it matches the image
...
`ensure_registry_session` (in `houba/use_cases/registry_session.py`), idempotent via a
...
- `--registry <name>` with an unknown name raises `ConfigError` (exit 3), consistent with the
  rest of the CLI.
- No change to the `HOUBA_REGISTRIES` schema or any port/adapter; the parity is purely at the
  use-case layer.
