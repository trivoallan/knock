# 28. Scan-referrer garbage collection (`knock gc`)
...
Every `knock attach` writes a fresh `application/vnd.knock.scan.result.v1` referrer onto the
...
A new `knock gc` verb walks the registry roster (sequentially, v1) and deletes superseded
...
`domain/retention`. Dry-run by default; `--apply` to delete; `KNOCK_DRY_RUN_DELETIONS` is the
deployment-wide kill-switch. No usage oracle — the decision is purely temporal/local. Registry-config
parity with reconcile / audit / purge / attach (`KNOCK_REGISTRIES` roster + `--registry` override,
