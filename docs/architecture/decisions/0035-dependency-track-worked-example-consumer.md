...
(it already names kind, OpenBao, ESO, Zot). The package-level blast-radius loop — knock's SPDX
...
consumer. knock attaches a CycloneDX SBOM referrer to every placed image (`KNOCK_SBOM_FORMATS`
...
- **DT is glue, not a feature.** No knock `DependencyTrack` adapter/port/use case; knock core
  is untouched (it just emits a CycloneDX referrer alongside SPDX — a config flag, ADR 0034).
  Currency/continuous-correlation remains out of knock's product scope — the demo wires a
  third-party tool, knock does not own it.

## Consequences

- The demo glue is two one-shot Jobs (API-key bootstrap, SBOM publish) plus the incident seed,
  all under `deploy/`. No custom image: `publish-sbom` runs the stock knock image (regctl +
