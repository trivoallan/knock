# 38. Closing the incident loop end-to-end does not make knock own runtime presence
...
(Dependency-Track reading knock's CycloneDX referrer, on the reproduced XZ / CVE-2024-3094 incident —
...
quietly grow knock into a fleet inventory — which would betray the stamper thesis and ADR 0032.

## Decision

The full incident answer is a **3-way join keyed on the image digest**, and knock is **one leg**:

- `CVE → digest` — Dependency-Track (reads the knock SBOM referrer). Shipped.
- `digest → owner` — the knock stamp annotation `io.knock.owners`, read by digest. knock's fact.
- `digest → cluster` — a **separate** runtime source: sandbox pods annotated `knock.io/image-digest`,
  listed via the kube API. **Not knock.**

knock supplies only the two digest-addressable facts (SBOM referrer + owner annotation). It does not
run the query and **does not own runtime presence**. The `digest → cluster` leg is the org's
runtime/observability plane, *demonstrated* by querying the kube API — never modelled as a knock
capability.

- **namespaces-as-clusters** is the sandbox stand-in for the cluster leg (namespaces labelled
  `knock.io/cluster=<name>`). The runtime pods run a node-pullable placeholder image
...
- **Zero knock-core.** The join is demo glue — extend `scripts/blast-radius.sh` + a read-only
...
- The bypass image (`bypassed/debian-xz`, never through knock) runs as a workload too, so its
...
- The C4 **Deployment** views (`DeployReference` / `DeployLocal`) gain the marked workload pods and
  **one new** edge — the consumer `→ the kube API` (`digest → namespace`); its existing read of the
  ...
  Context / Container / Component / Hexagon model is **unchanged** — knock core is untouched.
...
- The boundary of ADR 0032 is *re-affirmed by demonstration*: the loop closes end-to-end **without**
  knock crossing into runtime inventory or query.
