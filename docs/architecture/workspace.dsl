        ...
        refEnv = deploymentEnvironment "Reference — Argo App-of-Apps (the demo)" {
            ...
            deploymentNode "Kubernetes cluster" "kind (the demo) or a real cluster — same manifests (anti-drift)" "Kubernetes" {
                ...
                deploymentNode "namespace: houba" "houba workloads (wave 1)" "Namespace" {
                    ...
                    rfDt = infrastructureNode "Deployment: dependency-track (own app)" "Worked-example SBOM consumer (apiserver + frontend, embedded H2) — its own ArgoCD Application (ADR 0035). Currency layer: package-level blast-radius. DT is CycloneDX-only; houba's SPDX is converted by the publish glue." "Kubernetes Deployment"
            ...
            deploymentNode "Operator host" "Laptop / CI runner: runs kind, holds the policy clone" "macOS / Linux" {
                ...
                deploymentNode "kind cluster" "Single-node Kubernetes — overlay local (self-contained: buildkitd, no operators)" "kind" {
                    deploymentNode "namespace: houba" "houba workloads" "Namespace" {
                        ...
                        loDt = infrastructureNode "Deployment: dependency-track (own app)" "Worked-example SBOM consumer (apiserver + frontend, embedded H2) — (ADR 0035). Currency layer: package-level blast-radius. DT is CycloneDX-only; houba's SPDX is converted by the publish glue." "Kubernetes Deployment"
