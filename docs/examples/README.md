...
## SBOM — OCI referrer on every placed image

`houba reconcile` attaches a **package-level SBOM as an OCI referrer** on **every** image it places
— copy *and* rebuild alike. The SBOM is generated by a standalone `syft` invocation after the image
is placed (both paths know the placed digest at that point), and attached via the OCI referrers API
so `regctl image referrers` and any dist-spec-compliant tool discovers it:

```bash
# List referrers — see the SBOM referrer (artifactType: application/spdx+json)
regctl image referrers registry.example.com/demo/busybox@sha256:<digest>

# Fetch the SBOM
regctl manifest get registry.example.com/demo/busybox@sha256:<sbom-referrer-digest> --format body \
  | jq '.packages | length'
```

The format is controlled by `HOUBA_SBOM_FORMATS` (default `["spdx-json"]`). To emit CycloneDX
in addition to SPDX (one `syft` scan, two referrers):

```bash
HOUBA_SBOM_FORMATS='["spdx-json","cyclonedx-json"]' uv run houba reconcile docs/examples/reference
```

No `MirrorPolicy` field is involved — SBOM generation is a global, always-on policy enforced at the
operator level via the environment variable.
