...
SBOM attached as an OCI referrer. houba's digest-bound placement with the authoritative
`io.houba.owners=group:default/data-platform` label is exact — in contrast to the tag-only
"before" world (a raw, un-houba'd mirror under a project-named repo, where the owner is only
