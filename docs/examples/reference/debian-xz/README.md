...
knock, which attaches a **CycloneDX SBOM referrer** capturing `xz-utils 5.6.1-1`
(`KNOCK_SBOM_FORMATS` includes `cyclonedx-json`). `make publish-sbom` fetches that referrer and
uploads it, and Dependency-Track flags `DEBIAN-CVE-2024-3094`.

**What this does NOT claim.** knock does **not** detect or block the backdoor — nothing did, pre-
disclosure. knock rebuilds faithfully; the value is that on disclosure day, *"which images ship
...
registry — never through knock. It has no stamp and no SBOM, so the coverage report
(`make blast-radius`, the "⚠ carry NO knock stamp" line) flags it as the blind spot: what never came
