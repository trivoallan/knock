# Attach scan result — design (`houba attach`)

> **Status:** pre-implementation design, all forks resolved (§2, §10). Builds on the provenance
> *annotation* stamp (`houba/domain/stamp.py`) and the OCI **referrers** model. The terminal step
> after this spec is `writing-plans`.

## 1. Context & motivation

houba is a **stamper / single front door** for external OCI images. The roadmap's thesis is *"the
...
not a fleet-wide re-scan. So houba should carry the scan result in its stamp.

But houba is a **stamper, not a scanner**. *Running* a scanner is a commodity — CI pipelines already
run Trivy, registries (Harbor) ship native scanners, cloud scan services exist — and embedding it
would couple houba to scanner orchestration (the tool binary, a vulnerability DB to keep fresh,
...
**Therefore: the scan runs upstream; houba ingests its result and stamps it** as a portable,
standardized OCI referrer. The scan is the commodity; the stamp is the product. Even for images
houba *rebuilds* (a fresh digest no upstream CI has seen), the orchestration `reconcile → scan →
attach` belongs to the **pipeline around** houba, not inside it.
...
| Fork | Decision | Why |
|---|---|---|
| **Execution model** | **Ingest-only — houba never runs a scanner.** | Thesis (stamper, not scanner). Removes all scanner-orchestration coupling; standardized input formats make it tool-agnostic. |
...
| **Verb name** | **`houba attach`** — flat, no subcommand for now. | Honest: houba *attaches* an externally-produced result. `scan` as a verb would misrepresent (houba does not scan). Subcommand structure deferred until a second artifact kind needs it. |

## 3. Scope (v1)

**In:**

- A new verb **`houba attach <ref> --report <file|-> [--format <fmt>]`** that ingests a scan report
...
- **A `houba.attach` use case** returning a `ScanOutcome`, and a thin `cli/attach.py` command wired
  in `cli/_di.py`.
...
- **EOL / `regis`** as a third format mapper — a fast-follow; the registry accepts it with no core
  change (`io.houba.scan.eol.*`). This is the proof that the per-format design generalizes.
...
- **The downstream blast-radius query** — assembled in the org's observability stack from the
  referrers; not houba's job (roadmap: houba stamps, it does not watch).
- **Scan-at-import inside `reconcile`** — the pipeline orchestrates it around houba.
...
1. `houba attach <ref> --report <file|-> [--format <fmt>]`.
...
7. `build_scan_annotations(summary, prefix=…, subject_digest=…, format=…, timestamp=clock.now())`
   → the `io.houba.scan.*` annotation map.
...
- **`artifactType` = `application/vnd.houba.scan.result.v1`** — a stable filter key so consumers can
  list referrers of an image filtered to houba scan results (the query path). Versioned, generic,
  no org reference.
...
Honors `HOUBA_LABEL_PREFIX` (default `io.houba`); empty prefix ⇒ the report blob is still attached
but no `io.houba.scan.*` summary is written — consistent with `stamp.py`'s "empty prefix ⇒ no
labels".
...
| `{prefix}.scan.timestamp` | `2026-06-12T…Z` | `clock.now()` (when houba attached it) |
...
Namespacing by **finding type** (`vuln.*`), not by tool, is deliberate: `io.houba.scan.vuln.critical`
...
```
houba attach <ref> --report <path|->  [--format sarif]  [--output text|json]
```
...
sub-block (`HOUBA_SCAN_*`, own `env_prefix`, JSON-Schema-published) is the additive home — but YAGNI
keeps it out of v1.
...
- **Fakes:** add a journaling `put_referrer` to `tests/fakes/registry.py` (e.g.
  ...
  `artifactType=application/vnd.houba.scan.result.v1`, annotations including
  `io.houba.scan.vuln.critical`"*. No scanner fake is needed (no scanner port).
- **Integration:** add a `artifact put` scenario to the existing `tests/fake-bins/regctl` (branch on
  ...
  fake-bin** — houba runs no scanner.
...
- **C4 / `docs/architecture/workspace.dsl`** — add, at context level, an external system *"Upstream
  vulnerability scanner (CI / registry-native / scan service)"* that **produces the report houba
  ingests**, and the integration *houba → registry (attach referrer)*. Re-render + update the
  rationale in `docs/architecture/README.md`. (Note the direction of the new edge: the scanner feeds
  houba an artifact; houba does **not** call the scanner.)
- **`docs/examples/`** — add a `houba attach` walkthrough: a sample SARIF/Trivy report, the command,
  and the resulting referrer + `io.houba.scan.*` annotations. Mark *"requires the attach path"*
  until it lands.
...
- **JSON Schema** — publish the derived annotation-key vocabulary: the common envelope keys plus
  each mapper's declared `fact_keys`, so consumers/editors know the `io.houba.scan.*` contract.
...
- **Q (verb name) — `houba attach`, flat.** No subcommand now; if a second artifact kind appears
  (SBOM, VEX, EOL), revisit a `houba attach <kind>` grouping then, not pre-emptively.
...
`HOUBA_SCAN_SIGN`-style switch is set, the same report is signed (DSSE) and attached via the
