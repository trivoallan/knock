...
> stamp (`houba/domain/stamp.py`) and the catalog-walk pattern proven by `houba purge`
> (`use_cases/purge.py`). The terminal step after this spec is `writing-plans`.

## 1. Context & motivation

houba is a **stamper / single front door** for external OCI images. The roadmap's product
...
incident. houba's value is proportional to it being the **mandatory** path for external images.

Roadmap ④ makes that measurable: *"Show me the images in the registry that do **not** carry
houba's stamp"* — a coverage-gap report. This is what makes the front door **verifiable**, and
therefore **enforceable**: without it, "mandatory front door" is a wish. `houba audit` is that
report.
...
| **What "covered" means** | **The annotation stamp** | The universal mark houba writes on *every* processed image (copy **and** rebuild). Directly answers "which images lack the stamp". One read per image, a pure predicate. The signed-attestation layer (rebuild-only today) becomes a stricter coverage *tier* later — auditing it now would flag most legitimate houba (copy) images as "uncovered". |
...
**In:**
- A new **read-only** verb `houba audit [--registry NAME] [--fail-on-uncovered]`.
...
- **Attestation-coverage tier** (signed vs unsigned). A natural follow-up once the copy path is
  also attested; until then most houba images are unsigned-but-stamped, so a signed-coverage
...
- **Any mutation.** `houba audit` never writes — no stamping, no deletion, no referrers.

## 4. Architecture — hexagonal placement

The standard houba layering, mirroring `purge` (which is the closest sibling: a read-only
...
- **`domain/coverage.py`** (pure; ≥ 90 % coverage; `mypy --strict`). `is_stamped(annotations:
  dict[str, str], *, prefix: str) -> bool`: an image carries houba's stamp iff it shows the
  lineage houba writes. With a non-empty `prefix` (default `io.houba`), the definitive signal is
  the houba-namespaced lineage key **`{prefix}.policy`**. With an **empty** prefix (houba emits
  only OCI-standard keys), fall back to **`org.opencontainers.image.base.digest`** — the
  idempotency key houba writes in every case. Pure, no I/O.
  - *Known limitation (documented):* `org.opencontainers.image.base.digest` is an OCI-standard
    key, not houba-exclusive — a non-houba tool could in principle set it, so the empty-prefix
    fallback is a heuristic. The `{prefix}.policy` key (the default path) is the strong,
    houba-specific signal. This is acceptable: empty-prefix deployments deliberately trade the
    houba namespace away.
...
- **`cli/audit.py`** — Typer command `houba audit`, options `--registry NAME` (restrict to one
...
Rendered through the existing render layer (text + JSON, switched by `HOUBA_LOG_FORMAT`): a text
...
- **C4 / `docs/architecture/workspace.dsl`** — `houba audit` adds **no external system** (it
...
- **`docs/examples/`** — add a `houba audit` walkthrough to `docs/examples/README.md` (run it
...
- **Integration — `tests/integration/test_regctl_cli.py`** (extend): `get_annotations` against the
  ...
  → `RegctlError`.
- **CLI — `tests/integration/test_cli_audit.py`**: `houba audit` end-to-end against the fake-bin
  (report rendered; exit 0; `--fail-on-uncovered` flips the exit when an unstamped tag is present).

Coverage gates unchanged: ≥ 80 % global, ≥ 90 % on `houba.domain`.
