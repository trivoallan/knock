# `houba purge` — the reference reaper (usage-gated purge of marked tags)
...
The delegated-tag-deletion design (#41) lets houba **mark** an undesired tag with a `pending-deletion`
OCI referrer instead of hard-deleting it, then explicitly **delegates the verify-and-purge decision
to an external reaper** — because *"houba has no visibility into what is deployed"*. Its non-goals
state plainly: **"No reaper implementation … the reaper is an external system"** and **"houba never
...
This design fills that gap by shipping the reaper **as a houba subcommand**, `houba purge`, while
keeping it cleanly separable.

## 2. Goal & positioning

Ship `houba purge`: a **reference reaper** that
...
The roadmap fences off *"runtime presence / fleet inventory"* — **houba must not watch where its
images run.** A purge command that queries observability appears to cross that line. It does not, and
the distinction is load-bearing:

- **Forbidden:** houba *owning/maintaining* runtime state — a fleet inventory, a watcher, an operator.
- **This command:** a **stateless, point-in-time question** to the observability stack the thesis
  ...
  immediately and forgotten. houba stores nothing, watches nothing.

Asking ≠ watching. `houba purge` is the *logical endpoint* of the thesis (consume the observability
stack), not a violation of it. We therefore **revise #41's "the reaper is an external system"
non-goal honestly**: houba ships *a reference reaper*. To keep the thesis intact in both code and
...
- It **shares none of reconcile's orchestration** nor the build/transform/copy path; it reuses only
  ...
  case.
- It is documented as **replaceable**: the org may swap `houba purge` for their own reaper; the
...
```
houba purge [--registry <name>] [--apply]
```
...
- **Dry-run by default.** Without `--apply`, compute and print the plan (per candidate: `purge` /
  ...
  deletions. The plan is also suppressed-from-mutation when `HOUBA_DRY_RUN_DELETIONS=true` — for a
...
`PENDING_TYPE = application/vnd.houba.lifecycle.pending+json` (the artifactType #41 attaches).
...
and `--registry`/`HOUBA_REGISTRIES` already bound it. No new scaling mechanism is introduced.
...
Honest caveat, documented not fixed: on the **copy path** (no transform), the houba digest equals the
upstream digest, so a container that pulled the *same content directly from the upstream* (not from
houba's mirror) would also match. That is a **false-positive toward safety** — purge protects a tag
...
(e.g. require the running image's registry to be houba's) without a contract change.

## 5. Domain — `houba/domain/purge.py` (pure, ≥ 90 % coverage)
...
class MarkIdentity:                       # the 3-level stamp identity #41 writes into the mark;
    ...
    import_: str                         # only emits io.houba.policy/import/variant annotations).
...
- The threshold lives **only** in the domain (`usage_window_start`) — houba owns *"how many idle days
  is safe"*; the oracle script owns *"what counts as prod"* (§7.1). Clean split.

## 6. Ports & adapters

### 6.1 New port — `houba/ports/usage_oracle.py`
...
### 6.2 New adapter — `houba/adapters/command_usage.py`

`CommandUsageAdapter` drives a **configured subprocess** — the exact idiom of the regctl/buildctl
adapters, and the reason houba needs no HTTP layer for observability. Observability backends become
*configuration of a generic primitive*; "Datadog" is one script.

- **Invocation:** runs `HOUBA_USAGE_ORACLE_CMD` with a single **JSON object on stdin**:
...
- **Result:** the command prints **JSON on stdout**: `{"last_seen":"<ISO-8601>"}` or
  `{"last_seen":null}`.
- **Failure semantics:** exit `0` = answered; non-zero, timeout (`HOUBA_USAGE_ORACLE_TIMEOUT`), or
...
- `OperationKind` (`houba/ports/reporter.py`) gains **`purged`** and **`protected`**. `uncertain` is
...
- `houba/errors.py` gains **`UsageOracleError(AdapterError)`** → exit code **2** via the unchanged
  `exit_code_for` MRO walk.

## 7. Use case — `houba/use_cases/purge.py`
...
- **Oracle is mandatory.** If `HOUBA_USAGE_ORACLE_CMD` is unset, purge **refuses to run** with a
  `ConfigError` (exit 3) — you cannot verify, so you cannot purge. It never falls back to
  purge-everything.
- **`minIdle` has no default.** `HOUBA_PURGE_MIN_IDLE_DAYS` unset ⇒ `ConfigError`. The example's "15
  days" is the operator's value, never a houba default (a destructive default window is unsafe).
...
| Question | Owner |
|---|---|
| *"How many idle days is safe to purge?"* | houba domain + `HOUBA_PURGE_MIN_IDLE_DAYS` |
...
houba never encodes "prod". The script does. This is the CLAUDE.md rule ("org-specific … becomes
configuration of generic primitives") applied to observability.

## 8. Config — `houba/config.py`

New `HOUBA_*` settings (the only place env is read; JSON-in-a-var convention preserved):

- `HOUBA_USAGE_ORACLE_CMD: str | None = None` — the oracle command (required for purge; absent ⇒
  `ConfigError`).
- `HOUBA_USAGE_ORACLE_TIMEOUT: int` — per-call timeout seconds (sane default, e.g. 30).
- `HOUBA_PURGE_MIN_IDLE_DAYS: int | None = None` — the recency window; required for purge.
...
- **C4.** #41 introduced an *external reaper*; this design makes **houba itself the reference
  reaper** and introduces a **new external system — the usage oracle / observability stack —
  *queried by* houba**. `docs/architecture/workspace.dsl` is updated in the same change: the new
  external system, a `houba → oracle` "queries prod usage" relationship, and (Container/Component
  views) the `UsageOraclePort` + `CommandUsageAdapter` + `purge` use case + the new
  `RegistryPort.list_repositories`. The #41 "external reaper" actor is reconciled to houba's `purge`
  command. Mirrored as a thin ADR **`docs/architecture/decisions/0013-purge-reaper.md`** linking
  here.
- **Examples.** Add a `docs/examples/` walkthrough: a tag is marked (#41) → `houba purge` (dry-run)
...
- **Use-case tests** (`tests/unit/use_cases/`): with `FakeUsageOraclePort` (seed `last_seen` per
  ...
  deleted; dry-run (no `--apply`) mutates nothing; `--apply` deletes; unset `HOUBA_USAGE_ORACLE_CMD`
  or unset `HOUBA_PURGE_MIN_IDLE_DAYS` ⇒ `ConfigError`; partial failure exits non-zero without
  blocking siblings.
...
- Coverage gates unchanged: ≥ 80 % global, ≥ 90 % `houba.domain`. `cli/_di.py` excluded.
...
- **No in-tree Datadog (or any backend) client.** The oracle is a generic command; backends are
  scripts. No HTTP dependency enters houba.
...
- **Oracle correctness is the operator's.** A script that under-reports prod usage causes unsafe
  ...
  digest key is conservative (§4.1). The oracle's correctness is out of houba's control by design —
