...
ArgoCD want a copy-paste GitOps blueprint for houba. This spec adds one as a *variant* — it
demonstrates the App-of-Apps pattern and ships a reproducible kind demo, without promoting ArgoCD
to a requirement.

ArgoCD's role here (decision: option **D** — a documented variant): it manages the houba **install
...
deploy/argocd/
  ...
  apps/
    demo/
      ...
      houba.yaml                 # child Application → deploy/argocd/sources/houba-demo
    prod/
      ...
      houba.yaml                 # child Application → deploy/argocd/sources/houba-prod          [sync-wave 1]
  ...
  sources/
    ...
    houba-prod/kustomization.yaml    # ../../base + externalsecret + ClusterSecretStore (→ openbao) + image +
    ...
    buildkitd/kustomization.yaml     # buildkitd workload + houba's autoscaling config — references the existing
...
- **Demo = 2 children** (`registry` + `houba`, copy path) → real App-of-Apps mechanics. The registry
...
- **Prod = 6 children** (`eso` + `keda` + `prometheus` operators + `openbao` secret backend, then
  `houba` core + `buildkitd` rebuild/autoscaling add-on) → the prod root bootstraps the **entire** stack
...
- **`BUILDKIT_HOST` becomes a config literal** in `sources/houba-prod`'s `configMapGenerator`, set
  ...
  path works; don't → houba runs copy-only against a host that is simply never dialed.

This is why the ArgoCD variant composes houba **à la carte** (base + per-env patch) instead of
...
- **`prometheus`** — `kube-prometheus-stack` (`https://prometheus-community.github.io/helm-charts`),
  ...
  values to the minimum houba needs — `grafana.enabled=false`, `alertmanager.enabled=false`,
...
secret **data** is seeded by a documented demo step — a `bao kv put houba/registries
HOUBA_REGISTRIES=…` against the dev OpenBao pod (a Makefile/runbook action, **not** a committed
...
- **child Applications** — each `destination` = `kubernetes.default.svc` / namespace `houba` (registry
  child too — it lives in the houba namespace for the demo). `automated` sync. kustomize source.
...
```make
make demo-argocd        # kind up → build+load houba:dev → install argo-cd → patch argocd-cm → apply root → wait sync → run + blast-radius
...
1. `cluster` + `image` (reuse existing targets — kind create, build+load `houba:dev`).
...
The default kind demo is the **copy path** (`ARGOCD_ENV=demo`: registry + houba-demo) — fast and
...
- **Fork/branch ceiling** (intrinsic App-of-Apps ↔ git coupling, not a houba shortcut): ArgoCD reads
