...
- **Scope:** CI / release infrastructure. Add a `publish` job to `release-please.yml` that, on each
  release, builds and pushes a multi-arch (`amd64` + `arm64`) image to `ghcr.io/<owner>/knock` with
...
```
docker pull ghcr.io/<your-org>/knock:0.4
```

— a doc/reality gap, with an unresolved `<your-org>` placeholder. There is no published image to pull.

knock's product thesis is *"the label is the product"*, and it ships **signed SLSA / in-toto
...
2. **Mechanism — cosign-native (approach A).**
   ...
   knock dogfooding its own image, rejected as circular and outside the attestor's third-party-image
   use case. Both recorded in Out of scope.)*

3. **Architectures — `linux/amd64` + `linux/arm64`.**
   Cheap here: knock is pure-Python, so the built wheel is architecture-independent; only the
...
4. **Naming & tags.**
   Image reference `ghcr.io/${{ github.repository_owner }}/knock` — **generic in the YAML** (no org
   hardcoded; a fork publishes to its own namespace). The canonical public image is
   `ghcr.io/trivoallan/knock` (the repo owner, already present as `homepage` in
   ...
   consistent with *"the label is the product"*: knock's own image carries its own labels.
  ...
  publish:
    ...
    steps:
      ...
      - id: meta
        ...
        with:
          images: ghcr.io/${{ github.repository_owner }}/knock
          tags: |
      ...
      - name: Sign image (keyless)
        env:
          IMAGE: ghcr.io/${{ github.repository_owner }}/knock
          DIGEST: ${{ steps.build.outputs.digest }}
        run: cosign sign --yes "${IMAGE}@${DIGEST}"
      - name: Smoke test pushed image
        run: docker run --rm "ghcr.io/${{ github.repository_owner }}/knock@${{ steps.build.outputs.digest }}" version
```
...
- README **Install** section: resolve the placeholder to `ghcr.io/trivoallan/knock` and add a
  verification block:
  ```bash
  docker pull ghcr.io/trivoallan/knock:0.4
  cosign verify ghcr.io/trivoallan/knock:0.4 \
  ...
  docker buildx imagetools inspect ghcr.io/trivoallan/knock:0.4 --format '{{ json .SBOM }}'
  ```
...
- **Modify** `README.md` — Install section: resolve `<your-org>` → `ghcr.io/trivoallan/knock`; add the
...
2. **Post-release checklist** (run after the first release cut on this design):
   ...
   - `docker buildx imagetools inspect ghcr.io/trivoallan/knock:<v>` shows a multi-arch index **and**
...
`knock.domain`), `ruff`, `mypy knock` stay green.
...
- Approach C — knock stamping its own image (circular; outside the attestor's third-party use case).
