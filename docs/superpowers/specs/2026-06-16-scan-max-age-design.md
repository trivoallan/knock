...
ADR 0032 (PR #135) drew the boundary between knock and Dependency-Track: the `attach`
referrer is **signed scan provenance**, not a vuln store. An admission controller verifies the
*signature and digest binding* — *"this digest was scanned, signed by knock"* — but **never the
...
1. **Locus = producer-side.** knock *produces* a trustworthy freshness fact; the **admission
   controller enforces** the max-age policy against it. This is the ADR-0032-aligned locus ("the real
   gate is admission"). knock does **not** gain an audit-side freshness tier — an audit tier reports
...
2. **Clock = attach-time.** The freshness clock is the **existing** signed
   ...
   the frozen `/scan/v1` schema is **unchanged**. Semantics: *"knock (re)attached a scan at T"* — a
   provenance fact (knock's own signed claim about its own action). Rejected: a scanner report-time
...
knock's deliverable is to make an **existing** contract explicit and demonstrate it. **Zero new
...
1. **Contract prose.** Add `Field(description=...)` to `ScanPredicate.attested_at` in
   `knock/domain/scan/attestation.py`: the signed attach timestamp; the freshness clock an admission
...
3. **Precondition (documented, not built).** Max-age enforcement requires `KNOCK_ATTEST_SIGNER`:
...
4. **Worked example.** New `docs/examples/admission/` — a Kyverno policy (`verifyImages` +
   `attestations` of type `https://knock.dev/predicate/scan/v1` + temporal `conditions` that reject
...
5. **ADR 0033** (thin, follow-up to 0032): max-age = `attested_at`, enforced at admission; knock's
...
new happens in knock's runtime path.
...
**accepted**, mitigated by CI discipline (always scan-then-attach) and explicitly **not** knock's to
solve (non-goal).
