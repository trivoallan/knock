...
> knock-core change** (§4): it is deployment glue over reconcile's existing structured output
> (`Operation.out_digest`, [report.py](../../../knock/use_cases/report.py)) and the existing
> `knock attach`. Builds on the producer-side freshness decision in
...
~150k images.** knock is the front door (rebuild/harden, stamp, SBOM); `regis` governs (scan → SARIF
verdict). The missing piece is the **operational loop that runs the post-placement work** over each
image knock places: **`regis scan` → `knock attach` the signed verdict → publish the SBOM to
Dependency-Track.**

Two scoping facts fix the whole design:

- **Scope A — knock-placed images only.** The pipeline ensures *"every image knock placed is scanned /
  attached / in DT."* It does **not** hunt for images that bypassed the front door (a direct push to
  Harbor) — that is the separate `audit ④` question (B), §7. knock is the source of truth for A; the
  registry is not.
- **Target — all images.** knock's goal is to be the front door for **every** image. Today it manages
...
per-image probes `knock audit` runs — `image digest` + `manifest get`) measured **~1 s/image** against
...
of knock's deliberate "no HTTP layer"). Extrapolated to 150k: **~46 h base, ~115 h with
...
```
knock reconcile (CronJob, exists)            scan worker pool (glue)
  places/updates digests                       dequeue digest
  emits structured JSON with                     → regis scan
  out_digest per applied Operation               → knock attach (signed verdict)
...
- **Producer — `knock reconcile`** (the CronJob that already exists). Its output is a **structured JSON
  contract** (schema-published) whose every `Operation` carries `out_digest` (the produced digest, set
  iff `applied`). **The scan worklist is exactly the `out_digest`s of applied operations.** knock
...
- **Handoff — a durable work queue** (technology is deployment glue: Redis list / NATS / SQS / a DB
  ...
  At-least-once + dead-letter.
- **Consumer — a scan worker pool** (glue). Dequeue a digest → `regis scan` → `knock attach` → publish
  SBOM to DT → ack; on failure, retry, then DLQ after N. **Pool parallelism is the throughput knob**
  (gated by regis capacity, not by knock), a K8s setting.
...
knock (reconcile + the enqueuer glue) is the producer; the registry, regis and Dependency-Track are
...
C4Context
    ...
    System(knock, "knock reconcile", "Front door - places/updates digests; emits out_digest per applied Operation")
    ...
    Rel(platform, knock, "Configures policy")
    Rel(knock, registry, "Places digests; attaches stamp/SBOM")
    Rel(knock, queue, "Enqueuer pushes applied out_digests")
    Rel(queue, worker, "Dequeue digest")
    Rel(worker, regis, "Scan digest")
    Rel(worker, registry, "knock attach - signed verdict referrer")
...
sequenceDiagram
    autonumber
    participant R as knock reconcile (CronJob)
    ...
    W->>S: scan digest
    S-->>W: SARIF verdict
    W->>Reg: knock attach - signed scan-verdict referrer
    W->>DT: publish SBOM
    alt success
...
## 4. knock-core footprint — essentially none

The backbone is **pure deployment composition of existing knock verbs.** reconcile **already** emits
`out_digest` per applied `Operation` in a schema'd JSON contract; the worker **already** has
`knock attach` (and `regis` + a thin DT push). The enqueuer, the queue, the worker pool, the DLQ are
...
- The **`knock audit --scan` presence tier** (twin of `--signed`/`--sbom`, ADR 0036) — useful **only**
...
| **Enumerate via `_catalog` or the Harbor API** | **Not needed** — the backbone never re-discovers what knock placed; reconcile just placed it. Enumeration only matters for question B (§7). |
...
| **reconcile does scan inline** | Rejected — couples reconcile to the scanner, breaks **knock ≠ scanner**, and overloads the heavy placement path. reconcile emits; the worker scans. |
...
re-enqueue job — all deployment glue; reuse of reconcile's `out_digest` output and `knock attach`.
...
containers (enqueuer, worker), zero knock-core.

**workspace.dsl deferred to implementation, deferral recorded** (Kargo-spec precedent). The structural
delta is **deployment** level only — Dependency-Track, regis, and the `knock-reconcile` CronJob are
