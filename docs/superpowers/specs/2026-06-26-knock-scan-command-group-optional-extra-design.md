# Spec: `knock scan` command group (optional extra) + SBOM-publish stays a referrer-bridge

Status: **Draft**
Date: 2026-06-26
Relates to: #187 (platform scan pipeline), #188 (Redis Streams rebuild), #189 (redis:8 image bump)
ADR: [0043](../../architecture/decisions/0043-knock-scan-command-group-optional-extra.md)

## Summary

Promote the platform scan pipeline (shipped by #188 as `deploy/` + `scripts/` glue) into knock's
hexagonal layers as an **optional extra** (`pip install knock-oci[scan]`), while keeping core knock
`redis`-free. The SBOM-publish-to-Dependency-Track path stays a deployment-glue script bridge — the
OCI referrer remains knock's SBOM interface, no `SbomSinkPort` is built. Two independent cuts:
...
into `knock/domain/`, fronting the I/O with a `QueuePort` + `RedisStreamsAdapter`, and exposing a
`knock scan` Typer sub-app.

## Problem & context

#188 rebuilt the scan pipeline on Redis Streams under a deliberate "zero knock-core" boundary
(redis confined to the image; reconcile already emits the worklist). The question: should those
scripts — plus SBOM-publish-to-DT/GUAC — become a first-class `knock` command group? Drivers:
...
And SBOM-publish is a separate axis that touches knock's *product thesis*, not its substrate.
...
- **Scan pipeline → Approach D.** A `knock scan` command group behind a `QueuePort` +
  `RedisStreamsAdapter`, shipped as an **optional extra**, redis-py opt-in. Core knock stays
  redis-free. The pure logic comes under the `≥90%` domain gate + `mypy --strict`.
- **SBOM publish → Approach S-C.** The OCI referrer is knock's SBOM interface. The DT push stays a
  script bridge (`publish-sbom`); GUAC pulls (zero new knock code); `dt-bootstrap` stays glue. No
  `SbomSinkPort` unless "knock delivers to sinks" becomes a permanent product promise.

## Premises

1. The scripts are two populations (above); only the operator surface is human-typed.
2. CLI-ifying the worker into **core** would weld redis-py into knock-core and force it on every
...
3. The decision logic carries real, caught bugs (#188's three silent failures: trim evicting un-read
   ...
   trim footgun) that belong under knock's test gate.
4. The OCI referrer **is** knock's SBOM interface. `publish-sbom` exists only to compensate for DT's
...
5. DT and GUAC are asymmetric: DT needs a push (BOM_UPLOAD), GUAC pulls. Do not build a push client
   for a puller.
6. `dt-bootstrap` is vendor + k8s lifecycle glue. Never knock-core.

## Approaches considered

**Scan pipeline.** A) full integration into core (redis-py in CLI deps) — rejected, imposes redis on
all users, reverses zero-knock-core with no guard-rail. C) consolidate in `scripts/`, zero CLI —
preserves the boundary but no `knock scan` ergonomics and the pure logic never reaches the domain
...
**C4 Container view (after D + S-C).** The `knock-oci[scan]` extra is the new boundary; core knock stays
redis-free; the SBOM bridge + DT/GUAC sinks stay glue outside it.

```mermaid
flowchart TB
    operator["Operator<br/>[Person]"]
    reconcile["knock reconcile<br/>[CLI use-case]<br/>emits placed out_digests"]

    subgraph hexa["knock-oci[scan] — optional extra (core stays redis-free)"]
        direction TB
        cli["knock scan worker / dlq / coverage / enqueue<br/>[Typer sub-app, lazy redis import]"]
    ...
    subgraph glue["deploy/ glue — out of knock-core (S-C)"]
    ...
    operator -->|knock scan dlq replay| cli
    uc -->|regctl: fetch SBOM referrer| registry
    uc -->|knock attach: write SARIF referrer| registry
    uc -->|scan| scanner
...
sequenceDiagram
    ...
    alt SBOM referrer present and scan succeeds
        ...
        W->>S: scan(sbom)
        S-->>W: scan.sarif
        W->>Reg: knock attach (write SARIF referrer)
...
- **Domain (relocation):** `scripts/scan_queue.py` → `knock/domain/scan_queue.py`, essentially
  as-is; bring under `mypy --strict` and the `≥90%` gate; move its unit tests to `tests/unit/domain/`.
- **Port:** `knock/ports/queue.py` — `QueuePort` (Protocol) mirroring the verbs `scan_streams.py`
...
- **Adapter:** `knock/adapters/redis_streams.py` — `RedisStreamsAdapter` implementing `QueuePort`,
...
- **Use case:** `knock/use_cases/scan_worker.py` — the reserve→fetch-SBOM→scan→attach→ack loop. The
  worker **drives the scanner subprocess in-process** (captures `(stage, exit_code, stderr)` for
  `classify_failure`) and **calls the attach use case (`attach_scan`) in-process** (not a `knock
  ...
  the reaper/coverage *schedules* stay in `deploy/` (KEDA ScaledJob + CronJobs invoking `knock scan
  reaper` / `knock scan coverage`).
- **CLI + lazy registration:** `knock/cli/scan.py` builds a Typer sub-app registered via
  ...
  `ModuleNotFoundError` into a clean `pip install knock-oci[scan]` message. Subcommands: `worker`
  (`--check` preflight), `enqueue`, `reaper`, `coverage` (`--fail-on-gap[=PCT]`), `dlq
  {list|show|replay|drop}`.
- **Config:** route the flat `REDIS_*` env into `config.py` as a single `KNOCK_SCAN_REDIS` JSON var
  (roster pattern). **Loud migration:** if a stale flat `REDIS_*` var is still set after the switch,
  error (`REDIS_ADDR set but ignored; use KNOCK_SCAN_REDIS`) rather than silently using defaults.
...
- **Dependency:** `[project.optional-dependencies] scan = ["redis>=8"]`. The existing `redis>=5.0`
  ...
  unchanged.
- **Image + cutover:** install `.[scan]`; entrypoints become `knock scan worker/reaper/coverage/
  ...
  no other scan path).
- **Enqueue contract:** `knock reconcile --report-json | knock scan enqueue` (a new `--report-json`
...
- **Rewrite `publish-sbom.sh` → `scripts/publish-sbom.py`** (stays glue, not a knock command): stdlib
...
- Document GUAC: aim the collector at the registry; knock writes the CycloneDX referrer, GUAC
  ingests. No knock code.
...
Door 1 (two-way, internal): domain reloc + QueuePort + RedisStreamsAdapter + scan_worker
                            ...
                            + `knock scan worker/enqueue/reaper/coverage`; enqueue via stdin pipe
Door 2 (one-way, surface):  public `knock scan dlq` operator CLI, once the boundary proves out
```
...
> [ADR 0043](../../architecture/decisions/0043-knock-scan-command-group-optional-extra.md):
> 1. **The worker is split into `knock scan reserve` + `knock scan attach`**, not a single
>    `knock scan worker`. The #188 ScaledJob runs the scanner (grype) as a separate init-container
...
>    to "knock ≠ scanner"); D7's "worker drives the scanner" is deferred. Door 1 ships
>    `knock scan reserve/attach/enqueue/reaper`.
> 2. **`coverage` and `dlq` are Door 2.** They stay the existing `scripts/` (still on flat `REDIS_*`)
>    until the public operator CLI lands; the loud-migration guard only fires on `knock scan` pods,
>    which carry `KNOCK_SCAN_REDIS` exclusively.
...
- **`attested_at` is `clock.now()`, threaded via `ScanOutcome` — not a port extension.** Verified:
  ...
  digest — cosign surfaces no signed timestamp. So the "signed attested_at" is knock's own clock
  ...
  non-existent `knock verify --field attested_at`.) An earlier proposal to extend the attestor port
...
- No-redis CLI test: `knock scan --help` works + prints the install hint with redis-py absent.
...
- `knock scan --help` lists `worker/enqueue/reaper/coverage/dlq`; missing redis-py prints the install
  hint, not a stack trace. A test asserts this.
- Core `uv sync` (no extra) does **not** pull redis; `pip install knock-oci[scan]` does.
- `knock/domain/scan_queue.py` under `≥90%` + `mypy --strict`, #188 bug regressions preserved.
- `RedisStreamsAdapter` keeps the integration test green on real `redis:8`; ack/dead-letter ordering
  invariants asserted.
- Runtime image runs `knock scan worker` (+ CronJobs, enqueuer); argv shims + `scan_queue.py`/
  `scan_streams.py` deleted from `scripts/`; `scan-attach.sh` retained as backfill.
- In-flight `knock:scan:work`/`:dead` entries survive the entrypoint swap.
- DT publish unchanged (now `publish-sbom.py`); GUAC documented as pull; `dt-bootstrap` untouched.
- C4 + ADR + reference (+ example) + the `operate-scan-pipeline` runbook (rewritten to `knock scan
...
- **Extra naming:** `knock-oci[scan]` now; rename to `knock[platform]` only if the SBOM sink (S-D) or
  another platform concern lands.
- **Reaper/coverage scheduling:** the *logic* becomes `knock scan reaper`/`coverage`; the *schedule*
  stays the existing CronJobs (confirm they just swap their command).

## Distribution

The `knock-oci[scan]` optional extra **is** the distribution channel: `pip install knock-oci[scan]`, and the
