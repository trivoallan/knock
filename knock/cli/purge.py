"""The `knock purge` command — the reference reaper (spec §3)."""
...
from knock.cli._di import build_container, build_usage_oracle
from knock.errors import ConfigError
from knock.logging import configure
from knock.use_cases.purge import PurgeReport, purge_exit_code, purge_marks
...
def purge(
    ...
    if settings.purge_min_idle_days is None:
        raise ConfigError("knock purge requires KNOCK_PURGE_MIN_IDLE_DAYS (idle window in days)")
