"""The `knock scan` sub-app — platform scan-pipeline commands.

Redis is an optional dependency (``pip install knock-oci[scan]``).  Top-level imports
are stdlib + typer ONLY so that ``knock scan --help`` works without redis-py.
...
scan_app = typer.Typer(
    ...
    help="Platform scan-pipeline commands (requires `pip install knock-oci[scan]`).",
)
...
def _adapter() -> Any:
    ...
    try:
        ...
        typer.echo("pip install knock-oci[scan] (redis-py is not installed)", err=True)
        raise typer.Exit(code=3) from None
    from knock.cli._di import build_scan_adapter
...
def reserve() -> None:
    ...
    Path(os.environ.get("KNOCK_SCAN_DIGEST_PATH", "/shared/digest")).write_text(res.ref)
    Path(os.environ.get("KNOCK_SCAN_TOKEN_PATH", "/shared/reservation-token")).write_text(res.token)
    typer.echo(res.ref)
...
def attach() -> None:
    ...
    from knock.cli._di import build_scan_and_attach
    from knock.ports.queue import Reservation
    from knock.use_cases.scan_worker import handle_reservation

    ref = Path(os.environ.get("KNOCK_SCAN_DIGEST_PATH", "/shared/digest")).read_text().strip()
    token = (
        Path(os.environ.get("KNOCK_SCAN_TOKEN_PATH", "/shared/reservation-token"))
        .read_text()
        .strip()
    )
...
def enqueue() -> None:
    ...
    from knock.domain.scan_queue import enqueue_refs
