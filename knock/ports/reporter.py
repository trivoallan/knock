...
Pydantic RunReport tree in `knock.use_cases.report` embeds `Counts` and `ErrorInfo`.
"""
...
class ErrorInfo:
    ...
    message: str
    exit_code: int  # from knock.errors.exit_code_for
...
class Reporter(Protocol):
    ...
    `operation_applied` and `operation_failed` MAY be called concurrently from
    multiple worker threads; implementations MUST be thread-safe.
    """
