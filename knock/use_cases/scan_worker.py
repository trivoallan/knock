...
The ScaledJob init-container topology means:
  1. ``knock scan reserve``  — reserves one digest and writes token + ref to /shared
  2. [fetch-sbom + grype in separate containers, reading /shared/digest]
  3. ``knock scan attach``   — reads the reservation from /shared, attaches, acks
...
from knock.domain.scan_queue import classify_exception
from knock.ports.queue import QueuePort, Reservation
from knock.use_cases.attach import ScanOutcome, attach_scan
