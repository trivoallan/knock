...
#
# knock attach INGESTS a SARIF (--report); it does NOT run a scanner.
# grype runs first to produce the SARIF, then knock attach writes the
...
#   KNOCK_REGISTRIES  registry roster JSON (required for registry auth)
#
# Usage:
#   REG=localhost:5000 KNOCK_REGISTRIES='{"demo":{"host":"localhost:5000","tls_verify":false}}' \
...
# Configure regctl auth from KNOCK_REGISTRIES — mirrors scan-attach.sh's approach
# so that knock attach can write the scan referrer to the demo registry.
...
if not name:
    sys.exit("Cannot determine registry from KNOCK_REGISTRIES (set REG to match a host in the roster, or configure a single registry).")
r = roster[name]
...
# ${KNOCK} is "knock" inside the knock image (on PATH); on a host without knock
# installed, the caller sets KNOCK="uv run knock" (unquoted → word-splits to 3 args).
${KNOCK:-knock} attach "${REF}" --report "$WORK/scan.sarif" --fail-on "${THRESHOLD}"; rc=$?
case "$rc" in
