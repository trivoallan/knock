...
def test_put_referrer_is_journalled() -> None:
    ...
    reg.put_referrer("repo:6.0.0", "application/vnd.knock.lifecycle.pending+json", {"k": "v"})
    assert reg.marked == [
        ("repo:6.0.0", "application/vnd.knock.lifecycle.pending+json", {"k": "v"})
    ]
...
def test_list_referrers_returns_seeded_referrers() -> None:
    ref = Referrer(
        digest="sha256:ref1",
        artifact_type="application/vnd.knock.lifecycle.pending+json",
        annotations={},
        subject_tag="6.0.0",
    )
    reg = FakeRegistryPort(
    ...
    got = reg.list_referrers("repo:6.0.0", "application/vnd.knock.lifecycle.pending+json")
    assert got == [ref]
